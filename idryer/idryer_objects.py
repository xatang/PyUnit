...
class RealTimeMedianFilter:
    def __init__(self, window_size):
        ...
        logger.info(f"Initialized RealTimeMedianFilter with window_size={window_size}")
...
class iDryer_heater_fan(object):
    def __init__(self, name: str):
        ...
        self.speed: float = 0
        self.rpm: float = 0
        logger.info(f"Initialized iDryer_heater_fan with name='{name}'. Initial speed={self.speed}, initial rpm={self.rpm}")

    async def update(self, speed: float, rpm: float):
        logger.debug(f"Updating iDryer_heater_fan '{self.name}'. New speed={speed}, new rpm={rpm}")
        self.speed = speed
        self.rpm = rpm
        logger.debug(f"Updated iDryer_heater_fan '{self.name}'. Current speed={self.speed}, current rpm={self.rpm}")
...
class iDryer_heater(object):
    def __init__(self, name: str, max_temperature: float, fan: iDryer_heater_fan, moonraker_api: Moonraker_api):
        ...
        logger.info(f"Initialized iDryer_heater '{self.name}'. Max temperature={self.max_temperature}, initial temperature={self.temperature}, initial target={self.target}, initial power={self.power}")

    async def update(self, temperature: float, target: float, power: float):
        logger.debug(f"Updating iDryer_heater '{self.name}'. New temperature={temperature}, new target={target}, new power={power}")
        self.temperature = temperature
        self.target = target
        self.power = power
        logger.debug(f"Updated iDryer_heater '{self.name}'. Current temperature={self.temperature}, current target={self.target}, current power={self.power}")

    async def set(self, target: float, force: bool = False):
        heater_name = self.name.split(" ")[1]
        logger.debug(f"Attempting to set target temperature for iDryer_heater '{self.name}'. Requested target={target}, force={force}")
        ...
        if target <= self.max_temperature and self.target != target:
            ...
            if status != True:
                logger.error(f"Failed to set heater temperature for '{self.name}'")
            else:
                logger.debug(f"Successfully set target temperature for '{self.name}' to {target}")
        elif force == True:
            gcode = f"SET_HEATER_TEMPERATURE HEATER={heater_name} TARGET={target}"
            logger.debug(f"Sending G-code (forced): {gcode}")
            status = await self.moonraker_api.send_gcode(gcode=gcode)
            if status != True:
                logger.error(f"Failed to force-set heater temperature for '{self.name}'")
            else:
                logger.info(f"Successfully force-set target temperature for '{self.name}' to {target}")
...
class iDryer_temperature_sensor(object):
    def __init__(self, name: str, humidity_plateau_duration: int):
        ...
        self.relative_humidity_values: deque = deque(maxlen=humidity_plateau_duration)
        self.absolute_humidity_values: deque = deque(maxlen=humidity_plateau_duration)
        logger.info(f"Initialized iDryer_temperature_sensor '{self.name}'. Humidity plateau duration={humidity_plateau_duration}")
    ...
    def get_absolute_humidity(self, temperature: float = None, relative_humidity: float = None):
        ...
        saturation_vapor_pressure = 6.112 * (2.71828 ** ((17.67 * temperature) / (temperature + 243.5)))
        absolute_humidity = (saturation_vapor_pressure * relative_humidity * 2.1674) / (273.15 + temperature)
        absolute_humidity_rounded = round(absolute_humidity, 1)
        logger.debug(f"Calculated absolute humidity for '{self.name}'. Temperature={temperature}, relative_humidity={relative_humidity}, absolute_humidity={absolute_humidity_rounded}")
        return absolute_humidity_rounded

    async def update(self, temperature: float, relative_humidity: float):
        logger.debug(f"Updating iDryer_temperature_sensor '{self.name}'. New temperature={temperature}, new relative_humidity={relative_humidity}")
        
        self.temperature: float = round(temperature, 1)
        self.relative_humidity: float = round(relative_humidity, 1)
        self.absolute_humidity: float = self.get_absolute_humidity()
        
        self.median_relative_humidity = float(self.median_relative_humidity_filter.update(self.relative_humidity))
        self.median_absolute_humidity = float(self.median_absolute_humidity_filter.update(self.absolute_humidity))
...
class IDryer_servo_status(object):
    def __init__(self, status: int):
        ...
        logger.info(f"Initialized IDryer_servo_status with status={self.status} ({str(self)})")
    ...
    async def timeout_check(self):
        ...
        if delta > self.servo_state_change_timeout:
            logger.debug(f"Timeout check for IDryer_servo_status: timeout reached (delta={delta})")
            return True
        else:
            logger.debug(f"Timeout check for IDryer_servo_status: no timeout (delta={delta})")
            return False

    async def update(self, new_status: int):
        logger.debug(f"Updating IDryer_servo_status. New status={new_status} ({self.__str__()})")
        self.status = new_status
        self.last_state_change_time = time.time()
        logger.info(f"Updated IDryer_servo_status. Current status={self.status} ({self.__str__()})")
...
class iDryer_servo(object):
    def __init__(self, name: str, close_angle: float, open_angle: float, moonraker_api: Moonraker_api):
        ...
        logger.info(f"Initialized iDryer_servo '{self.name}'. Close angle={self.close_angle}, open angle={self.open_angle}")

    async def close(self, ignore_timeout: bool = False):
        logger.debug(f"Attempting to close servo '{self.name}'. Ignore timeout={ignore_timeout}")
        if ignore_timeout == False:
            if await self.status.timeout_check() == False:
                logger.debug(f"Servo '{self.name}' close operation skipped due to timeout check")
                return
        if self.status == 0:
        ...
        logger.info(f"Servo '{self.name}' closed. Target angle set to {self.target_angle}")

    async def open(self, ignore_timeout: bool = False):
        logger.debug(f"Attempting to open servo '{self.name}'. Ignore timeout={ignore_timeout}")
        if ignore_timeout == False:
            if await self.status.timeout_check() == False:
                logger.debug(f"Servo '{self.name}' open operation skipped due to timeout check")
                return
        if self.status == 1:
        ...
        logger.info(f"Servo '{self.name}' opened. Target angle set to {self.target_angle}")
        ...
        while True:
            if self.target_angle > self.current_angle:
                while self.current_angle < self.target_angle:
                    logger.debug(f"Soft-setting servo '{self.name}'. Increasing angle from {self.current_angle} to {self.current_angle + step}")
                    await self.set(self.current_angle + step)
                    await asyncio.sleep(sleep)
                delta = self.current_angle - self.target_angle
            else:
                while self.current_angle > self.target_angle:
                    logger.debug(f"Soft-setting servo '{self.name}'. Decreasing angle from {self.current_angle} to {self.current_angle - step}")
                    await self.set(self.current_angle - step)
                    await asyncio.sleep(sleep)
                delta = self.target_angle - self.current_angle
            if delta <= step and delta != 0:
                logger.debug(f"Final adjustment for servo '{self.name}'. Setting angle to {self.target_angle}")
    ...
    async def set(self, angle: int):
        ...
        if status == True:
            ...
            logger.debug(f"Successfully set servo '{self.name}' angle to {angle}")
            return True
        else:
...
class iDryer_led_pixel(object):
    def __init__(self, led_name: str, index: int, red: int, green: int, blue: int, moonraker_api: Moonraker_api):
        ...
        logger.info(f"Initialized iDryer_led_pixel '{self.led_name}' at index {self.index}. Initial RGB: ({self.red}, {self.green}, {self.blue})")

    async def update(self, red: int, green: int, blue: int):
        logger.debug(f"Updating iDryer_led_pixel '{self.led_name}' at index {self.index}. New RGB: ({red}, {green}, {blue})")
        self.red = red
        self.green = green
        self.blue = blue
        logger.debug(f"Updated iDryer_led_pixel '{self.led_name}' at index {self.index}. Current RGB: ({self.red}, {self.green}, {self.blue})")

    async def set(self, red: int, green: int, blue: int):
        logger.debug(f"Setting iDryer_led_pixel '{self.led_name}' at index {self.index}. New RGB: ({red}, {green}, {blue})")
        self.red = red
        self.green = green
        self.blue = blue
        ...
        status = await self.moonraker_api.send_gcode(gcode=gcode)
        if status != True:
            logger.error(f"Failed to update LED pixel '{self.led_name}' at index {self.index}")
        else:
            logger.debug(f"Successfully updated LED pixel '{self.led_name}' at index {self.index}")

    async def get_rgb(self):
        rgb = [self.red, self.green, self.blue]
        logger.debug(f"Retrieved RGB values for iDryer_led_pixel '{self.led_name}' at index {self.index}: {rgb}")
        return rgb
...
class iDryer_led(object):
    def __init__(self, name: str, brightness: int, moonraker_api: Moonraker_api):
        ...
        self.moonraker_api: Moonraker_api = moonraker_api
        logger.info(f"Initialized iDryer_led '{self.name}'. Initial brightness={self.brightness}%")

    async def update(self, color_data_raw: list):
        logger.debug(f"Updating iDryer_led '{self.name}'. New color data: {color_data_raw}")
        led_pixels = []
        color_counter = 1
        for color_data_item in color_data_raw:
            ...
            if len(self.led_pixels) != len(color_data_raw):
                logger.debug(f"Creating new LED pixel for '{self.name}'. Index={index}, RGB=({red}, {green}, {blue})")
                ...
                for self_pixel in self.led_pixels:
                    if self_pixel.index == index:
                        logger.debug(f"Updating existing LED pixel for '{self.name}'. Index={index}, RGB=({red}, {green}, {blue})")
                        await self_pixel.update(red=red, green=green, blue=blue)
                        break
            color_counter += 1
        if len(self.led_pixels) != len(color_data_raw):
            self.led_pixels = led_pixels
            logger.debug(f"Updated LED pixels for '{self.name}'. Total pixels={len(self.led_pixels)}")
    ...
    async def set_color(self, index: int, red: int, green: int, blue: int):
        ...
        logger.debug(f"Setting color for LED '{self.name}' at index {index}. Adjusted RGB=({red}, {green}, {blue})")
        for led_pixel in self.led_pixels:
            if led_pixel.index == index:
                current_rgb = await led_pixel.get_rgb()
                if current_rgb != [red, green, blue]:
                    logger.debug(f"Updating LED pixel '{self.name}' at index {index}. Current RGB={current_rgb}, New RGB=({red}, {green}, {blue})")
                    await led_pixel.set(red=red, green=green, blue=blue)
                else:
                    logger.debug(f"LED pixel '{self.name}' at index {index} already has the desired RGB=({red}, {green}, {blue})")
                break
...
class iDryer_status(object):
    def __init__(self, status: int):
        self.status = status
        logger.info(f"Initialized iDryer_status with status={self.status} ({self.__str__()})")
    ...
    async def update(self, new_status: int):
        logger.debug(f"Updating iDryer_status. New status={new_status} ({self.__str__()})")
        self.status = new_status
        logger.info(f"Updated iDryer_status. Current status={self.status} ({self.__str__()})")
...
class IDryer_heater_PID(object):
    def __init__(self):
        ...
        logger.info(f"Initialized IDryer_heater_PID. PID parameters: Kp={self.pid.Kp}, Ki={self.pid.Ki}, Kd={self.pid.Kd}, setpoint={self.pid.setpoint}, output_limits={self.pid.output_limits}")

    async def update(self, target_temperature: float, min_temperature: float, max_temperature: float):
        logger.debug(f"Updating PID controller. New target_temperature={target_temperature}, min_temperature={min_temperature}, max_temperature={max_temperature}")
        ...
        if self.pid.output_limits[0] != min_temperature or self.pid.output_limits[1] != max_temperature:
            logger.debug(f"Updating PID output limits to ({min_temperature}, {max_temperature})")
    ...
    async def get(self, current_temperature: float):
        ...
        logger.debug(f"Calculated PID output for current_temperature={current_temperature}. Output={output}")
        return output
...
class IDryer_humidty_PID(object):
    ...
    async def get(self, curent_humidity: float):
        logger.debug(f"Calculating PID output for current_humidity={curent_humidity}")
        ...
        logger.debug(f"Calculated target_temperature={target_temperature_rounded} for current_humidity={curent_humidity}")
...
class iDryer(object):
    def __init__(self, id: int, name: str, moonraker_api: Moonraker_api, heater: str, heater_fan_name: str, heater_max_temperature: float, temperature_sensor: str, servo_name: str, servo_close_angle: float, servo_open_angle: float, led: str, led_brightness: int, presets: list[iDryer_preset], humidity_open_treshold: float, humidity_close_treshold: float, humidity_plateau_duration: int, humidity_plateau_window_size: int, humidity_timer_drying_range: float):
        ...
        logger.info(f"Initialized iDryer '{self.name}' (ID={self.id}). Components: heater='{heater}', heater_fan='{heater_fan_name}', temperature_sensor='{temperature_sensor}', servo='{servo_name}', led='{led}'")
