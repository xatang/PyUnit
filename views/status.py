...
class StatusHandler(web.View):
    async def get(self):
        ...
        self.request.app.logger.info(
            f"Handling GET request for iDryer status (ID={idryer_id}) from {self.request.remote}")
        idryers = self.request.app.idryers
        for idryer in idryers:
            if idryer_id == idryer.id:
                self.request.app.logger.debug(
                    f"Found iDryer with ID {idryer_id}. Returning JSON response")
                return web.json_response(idryer.to_json())
        self.request.app.logger.warning(
            f"iDryer with ID {idryer_id} not found. Returning 404")
        return web.Response(status=404)

    async def post(self):
        idryer_id = int(self.request.match_info.get("idryer_id"))
        self.request.app.logger.info(
            f"Handling POST request for iDryer status (ID={idryer_id}) from {self.request.remote}")
        data = await self.request.json()
        id = data.get('id')
        if idryer_id != id:
            self.request.app.logger.warning(
                f"ID mismatch in request. Expected {idryer_id}, got {id}. Returning 405")
        ...
        if preset_id != None:
            try:
                ...
                self.request.app.logger.warning(
                    f"Invalid preset_id value: {preset_id}. Returning 406")
                return web.Response(status=406)
        ...
        if custom_preset != None:
            try:
                ...
                max_temperature_delta = float(
                    custom_preset['max_temperature_delta'])
                ...
                self.request.app.logger.warning(
                    f"Invalid custom_preset data. Returning 406")
                return web.Response(status=406)
            ...
            if (temperature < 0 or max_temperature_delta < 0 or humidity < 0 or
                ...
                self.request.app.logger.warning(
                    f"Invalid custom_preset values. Returning 406")
                return web.Response(status=406)
            ...
            self.request.app.logger.debug(
                f"Created custom preset: {custom_preset}")
        ...
        for idryer in idryers:
            if idryer_id == idryer.id:
                self.request.app.logger.debug(
                    f"Changing status for iDryer (ID={idryer_id}) to {status}")
